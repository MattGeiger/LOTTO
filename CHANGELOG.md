...
- **Dashboard bottom-nav tab:** added a new **Dashboard** destination between
  **Your ticket** and **What's in stock** in the public bottom navigation,
  linking to `/display`. The core bar uses a local animate-ui
  `layout-dashboard` icon through the existing imperative-ref nav adapter, the
  Arcade bar gets a matching pixel-art dashboard glyph, and `/display` now
  renders the bottom nav with scroll clearance and a lower desktop dock offset
  for the dense public board.
