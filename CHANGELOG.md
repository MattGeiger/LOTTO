...
- **St. Johns Food Share queue-only profile.** Added the authorized agency logo,
  ...
  treatment in dark mode, agency copy, install identity, and a deliberately
  compact color system built from brand teal (`#33A478`), off-white (`#F7F7F7`),
  and charcoal (`#2D2D2D`). Neutral surfaces now carry the layout while teal is
  reserved for emphasis. The operational **Now Serving** state uses `#319A72`
  in light mode and luminous mint in dark mode;
