...
// so these pure functions can be unit-tested and imported by client components
// (e.g. the markdown renderer's heading IDs + link rewriting).
...
export function parseFilename(filename: string): { slug: string; order: number } | null {
...
export function getGuideSearchEntries(guide: UserGuide): GuideSearchEntry[] {
  const slugger = createGuideSlugger();
