...
export function HelpSearch({ index, className }: HelpSearchProps) {
  ...
  const terms = React.useMemo(() => getSearchTerms(query), [query]);
      ...
      {showPanel ? (
        <div className="absolute left-0 right-0 top-11 z-50 overflow-hidden rounded-lg border bg-popover text-popover-foreground shadow-md">
          {trimmedQuery.length < MIN_QUERY_LENGTH ? (
            ...
            <ul className="max-h-[min(28rem,70vh)] overflow-y-auto py-1" role="listbox" aria-label="Help search results">
              {results.map((result) => {
                const href = `/help/${result.guideSlug}?q=${encodeURIComponent(trimmedQuery)}#${result.sectionId}`;
                  ...
                  <li key={result.id} role="option" aria-selected={false}>
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-0.5 block truncate text-xs text-muted-foreground">{result.guideTitle}</span>
            ...
            <p className="px-3 py-4 text-sm text-muted-foreground">No help sections match &quot;{trimmedQuery}&quot;.</p>
          )}
        </div>
      ) : null}
    </div>
