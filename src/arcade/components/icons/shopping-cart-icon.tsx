import type { ComponentPropsWithoutRef } from "react";

// Pixel-art shopping-cart glyph (arcade nav: "What's in stock"). Geometry from
// src/arcade/lucid_icons/SVG/Flat/shopping-cart.svg; fill set to currentColor.
export function ShoppingCartIcon(props: ComponentPropsWithoutRef<"svg">) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={24}
      height={24}
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      {...props}
    >
      <polygon points="9 19 10 19 10 21 9 21 9 22 7 22 7 21 6 21 6 19 7 19 7 18 9 18 9 19" />
      <polygon points="20 19 21 19 21 21 20 21 20 22 18 22 18 21 17 21 17 19 18 19 18 18 20 18 20 19" />
      <path d="m4,3v-1H1v2h3v3h1v5h1v4h1v1h13v-2h-12v-2h12v-1h1v-3h1v-3h1v-3H4Zm16,3v3h-1v2H7v-4h-1v-2h15v1h-1Z" />
    </svg>
  );
}
