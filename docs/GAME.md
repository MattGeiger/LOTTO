...
- Completed: **Star Swarm** — the Arcade's third game, a Space Invaders-lineage fixed-shooter. Page at `/arcade/star-swarm` with shell, `arcade-swarm-*` CSS (square 224×224 board), 8-locale translations under `starSwarm*`, and an Arcade menu entry. Pure-function engine under `src/arcade/game/star-swarm/` (`types`, `constants`, `sprites`, `engine`, `renderer`) on the same `requestAnimationFrame` fixed-timestep architecture as Brick Mayhem. Full design doc in `docs/STAR_SWARM.md`.
- Completed: Star Swarm gameplay — a 5×8 descending invader formation that speeds up as it thins, destructible bunkers, invader bombs (shootable mid-air), a periodic bonus saucer (50–300 pts), tiered row scoring (40/30/20/10/10), post-hit invulnerability blink, endless waves, and a 6-tier difficulty system (Very Easy → Nightmare) tuning march cadence and bomb frequency.
- Completed: Star Swarm mobile controls — sticky control dock with a thumb movement slider + hold-to-autofire FIRE button, plus keyboard ←/→ and Space/↑; ticket-called auto-pause and play-resumed dispatch; theme-aware canvas palette; button-only haptics (collisions visual-only on the web path).
- Completed: Star Swarm engine coverage in `tests/arcade-star-swarm-engine.test.ts` — shot/invader scoring, wave-clear and next-wave progression, bomb-vs-ship life loss + invulnerability, game-over on last life and on formation breach, and shot cooldown / on-screen shot cap.
- Completed (v1.11.0): **Re-themed Star Swarm into Zombie Attack!** — renamed the route, modules, CSS, and translation keys (`/arcade/star-swarm` → `/arcade/zombie-attack`, `src/arcade/game/star-swarm/*` → `zombie-attack/*`, `arcade-swarm-*` → `arcade-zombie-*`, `starSwarm*` → `zombieAttack*`); design doc moved to `docs/ZOMBIE_ATTACK.md`. Star Swarm history above is retained as the record of what shipped in v1.10.0.
- Completed (v1.11.0): Zombie Attack content — three zombie builds (skinny/ribs/fat, 2-frame shamble) replacing the aliens, a dirt-lot background replacing the starfield, a **fence** in front of the bunkers that collapses under horde pressure (reaching the bunker line is game over), a **flaming truck** that descends top→bottom and collapses the fence on impact (3–5 shots to destroy), and **bomb-carrier zombies** that drop a bomb when shot which detonates a ~25%-of-board blast when shot.
- Completed (v1.11.0): Zombie Attack difficulty/UX — Nightmare has no bunkers (fence remains; bunker location is still the death line), Very Easy makes bunkers bomb-proof (player shots still erode); board made 25% taller (`224×280`, `aspect-ratio 224/280`) with re-tuned responsive sizing; fire control simplified to a compact hold-to-autofire **"A"** button with a wider movement slider; `user-select: none` + `touch-action: none` mask text selection on the Play/Fire/dock surfaces. Engine coverage rewritten as `tests/arcade-zombie-attack-engine.test.ts` (11 cases).
- Completed (v1.12.0): **Zombie Attack top-down survival overhaul** — board re-laid to a tall 240×360 top-down view; switched rendering to preloaded **NES-era PNG sprites** via `drawImage` (assets in `src/arcade/game/zombie-attack/assets/`, imported + preloaded in `assets.ts`; removed `sprites.ts`). Zombies now spawn at the top and **descend with a stochastic per-step direction** (50% down / 25% / 25% diagonals); civilians (4 sprite variants) don't shoot. Added the **Bub** soldier zombie (2 HP, stochastic 1911 fire that costs the hero a life, 50% grenade drop → shootable AoE blast), a top-down **Uzi hero**, an **ambulance** hazard (shoot to explode + clear zombies), and a **bunker-integrity** breach buffer.
- Completed (v1.12.0): Zombie Attack **helicopter rescue / timed rounds** — a helipad + animated helicopter drive a 4-round timed cycle (clear pad → refuel → board → takeoff); surviving the round clocks advances it, and completing round 4 extracts the chopper (rescue), clears the lot, repairs the line, and loops at higher difficulty. New HUD (objective banner, ROUND/TIME/LIVES/SCORE, round-timer bar). Difficulty presets retuned for spawn rate / descent speed / Bub frequency / bunkers / ambulance HP. Engine test rewritten (`tests/arcade-zombie-attack-engine.test.ts`, 9 cases) with an RNG seam for determinism.
