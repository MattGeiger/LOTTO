...
- **WITHIN existing patterns:** Optional client-device haptics are route-scoped to Arcade and `/new`; public-board routes remain silent.
...
- Optional client-device haptics are available on Arcade and `/new` only for direct button-style interactions; `/` and `/display` remain haptic-free.
