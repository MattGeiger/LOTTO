...
describe("admin email policy", () => {
  ...
  it("combines exact addresses with an allowed domain", () => {
    ...
    expect(isAdminEmailAllowed("queue-manager@example.org", env)).toBe(true);
  ...
  it("supports an exact domain policy without an allowlist", () => {
    const env = {
      NODE_ENV: "production",
      ADMIN_EMAIL_DOMAIN: "example.org",
    };

    expect(isAdminEmailAllowed("staff@example.org", env)).toBe(true);
    expect(isAdminEmailAllowed("staff@notexample.org", env)).toBe(false);
    expect(isAdminEmailAllowed("staff@example.org.example.com", env)).toBe(false);
  });
